...
def test_word_level_diff_minimal_spans():
    ...
    old_changed = [p for p in old_parts if p.get("type") != "equal"]
    ...
    new_changed2 = [p for p in new_parts2 if p.get("type") != "equal"]
    
    # should only have one changed part (the word "some" removed)
    assert len(old_changed2) == 1, f"Expected 1 changed part, got {len(old_changed2)}: {old_changed2}"
...
def test_apostrophe_normalization():
    ...
    text_no_apostrophe = "whats"
    
    normalized_curly = _normalize_text_for_diff(text_with_curly.replace("'", "\u2019"))  # replace with curly
    normalized_straight = _normalize_text_for_diff(text_with_straight)
    normalized_no_apostrophe = _normalize_text_for_diff(text_no_apostrophe)
    ...
    assert normalized_curly.lower() == normalized_straight.lower(), \
    ...
    normalized_quotes = _normalize_text_for_diff(text_with_quotes.replace('"', '\u201C').replace('"', '\u201D'))
    normalized_straight_quotes = _normalize_text_for_diff(text_with_quotes)
    
    assert normalized_quotes == normalized_straight_quotes, \
...
def test_line_anchor_hello_insertion():
    ...
    assert len(equal_segments) >= 2, f"Expected at least 2 equal segments, got {len(equal_segments)}"
    ...
    assert added_segments[0].get("new_lines") == ["hello"], \
...
def test_single_word_removal_within_stable_paragraph():
    ...
    """
    QUOTE_LINE_BASELINE = "This is a long quotation paragraph with whats many words that should remain unchanged."
    QUOTE_LINE_CURRENT = "This is a long quotation paragraph with many words that should remain unchanged."
    ...
    assert len(modified_segments) == 1, \
    ...
    assert len(removed_segments) == 0, \
        f"Expected 0 removed segments, got {len(removed_segments)}"
    assert len(added_segments) == 0, \
        f"Expected 0 added segments, got {len(added_segments)}"
    ...
    assert "whats" in removed_parts[0].lower(), \
...
def test_apostrophe_normalization_in_diff():
    ...
    assert len(equal_segments) == 1 or len(modified_segments) == 1, \
    ...
    if len(modified_segments) == 1:
        ...
        assert len(equal_parts_old) > 0 and len(equal_parts_new) > 0, \
    ...
    assert len(modified_segments2) == 1, \
    ...
    assert len(equal_parts) > 0, \
        "Should have equal parts even when apostrophe is removed"
...
def test_jaccard_similarity():
    ...
    assert similarity2 >= 0.6, \
        f"Expected similarity >= 0.6 for high overlap, got {similarity2}"
    ...
    assert similarity3 == 0.0, \
        f"Expected similarity 0.0 for no overlap, got {similarity3}"


def test_no_phantom_formatting_changes_after_content_only_edits():
    """
    Acceptance test: No phantom formatting changes after content-only edits
    Arrange: perform edits (hello insertion, whats removal) with no styling changes.
    Expect: zero formatting-change entries.
    
    Note: This test verifies the logic that prevents phantom formatting changes.
    The actual formatting comparison happens in the diff endpoint, but we can test
    that the diff segments don't incorrectly mark unchanged text as modified.
    """
    # simulate the scenario: insert "hello" and remove "whats" with no formatting changes
    baseline_lines = [
        "add",
    ...
    # check that unchanged lines are marked as equal, not modified
    equal_segments = [s for s in diff_segments if s.get("type") == "equal"]
    modified_segments = [s for s in diff_segments if s.get("type") == "modified"]
    
    # "add" should be equal
    add_in_equal = any("add" in (s.get("old_lines") or []) for s in equal_segments)
    assert add_in_equal, "Line 'add' should be marked as equal, not modified"
    
    # the long paragraph should have minimal changes (only "whats" removed)
    # it should be marked as modified (since a word was removed), but with word-level diffs
    paragraph_segments = [s for s in modified_segments if any(
        "quotation paragraph" in line for line in (s.get("old_lines") or [])
    )]
    
    if paragraph_segments:
        # if the paragraph is marked as modified, it should have word-level diffs
        paragraph_seg = paragraph_segments[0]
        old_char_diffs = paragraph_seg.get("old_char_diffs", [[]])[0]
        
        # should have equal parts (most of the paragraph is unchanged)
        equal_parts = [p for p in old_char_diffs if p.get("type") == "equal"]
        assert len(equal_parts) > 0, \
            "Modified paragraph should have equal parts (most text unchanged)"
...
def test_word_level_diff_preserves_unchanged_text():
    ...
    assert old_equal_text.lower().strip() == new_equal_text.lower().strip(), \
