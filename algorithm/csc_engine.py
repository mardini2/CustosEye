...
def _looks_hexish(s: str) -> bool:
    ...
    if len(alnum) < 8:  # if there are not enough alphanumeric chars
        return False  # cannot be hexish
    return (
        sum(1 for ch in alnum if ch in hex_chars) / len(alnum) >= 0.9
    )  # return true if 90 percent or more are hex chars
...
class CSCTrustEngine:
    ...
    def _load_weights(self) -> dict[str, Any]:
        ...
        defaults: dict[str, Any] = {
            ...
            "system_paths": [
                "\\windows\\system32",
                "/windows/system32",
            ],  # paths that indicate system executables
            "program_files_paths": [
                "\\program files",
                "\\program files (x86)",
            ],  # paths that indicate installed programs
            "temp_paths": [
            ...
            "misspell_tokens": [
                [
                    "svchost",
                    "svhost",
                ],  # pairs of [correct spelling, misspelling] to catch typos of system processes
                ["explorer", "exploer"],
                ["chrome", "chr0me"],
                ["system", "syst3m"],
                ["microsoft", "micros0ft"],
            ...
            "risky_ports": [
                ...
                139,
                445,
            ],  # ports that are commonly used for attacks
            "common_ports": [
                80,
                443,
                53,
                ...
                587,
                993,
            ],  # ports that are commonly used for legitimate purposes
            ...
            "class_rules": {
                ...
                "system": {
                    "if_system_dir": True,
                    "if_signed": True,
                },  # if in system dir and signed, classify as system
                "service": {"if_service": True},  # if it is a service, classify as service
                "dev_tool": {
                    "if_parent_is_dev_shell": True
                },  # if launched by dev shell, classify as dev_tool
                "game": {
                    "if_publisher_any": ["valve", "epic", "unity"]
                },  # if publisher is game company, classify as game
                "popular_app": {
                    "if_publisher_any": ["google", "mozilla", "adobe", "oracle"]
                },  # if publisher is well-known, classify as popular_app
            },
        }
        try:
            with open(
                self.weights_path, encoding="utf-8"
            ) as f:  # try to load the weights config file
            ...
            if isinstance(overrides, dict):  # make sure we got a dictionary
                ...
                for k, v in overrides.items():  # loop through each key-value pair in the overrides
                    if isinstance(v, dict) and isinstance(
                        defaults.get(k), dict
                    ):  # if both override and default are dicts
    ...
    def _days_since(self, ts: float) -> float:
        ...
        return max(
            0.0, (_now() - float(ts)) / 86400.0
        )  # calculate days since timestamp (86400 seconds per day)
    ...
    def _decayed_seen(self, seen: int, last_seen: float, halflife_days: float) -> float:
        ...
        return float(seen) * (
            0.5 ** (days / float(halflife_days))
        )  # apply exponential decay: count halves every halflife_days
    ...
    def evaluate(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        parent_name = _safe_lower(
            event.get("parent_name") or event.get("parent")
        )  # parent process name
        ...
        signer_subject = _safe_lower(
            event.get("signer_subject") or ""
        )  # certificate subject (publisher name)
        ports = event.get("listening_ports") or []  # ports this process is listening on
        remotes = (
            event.get("remote_addrs") or event.get("remote_endpoints") or []
        )  # remote connections
        ...
        in_system = bool(
            exe and any(seg in exe for seg in w["system_paths"])
        )  # check if path contains system directory
        in_pf = bool(
            exe and any(seg in exe for seg in w["program_files_paths"])
        )  # check if path contains Program Files
        in_userish = bool(
            exe
            and any(
                seg in exe for seg in (w["temp_paths"] + w["downloads_paths"])
            )  # check if path is in temp or downloads
        )
        ...
        if signer_valid:  # if the executable has a valid code signature
            ...
            for vendor, bucket in w["publisher_buckets"].items():  # loop through known publishers
                if vendor in signer_subject:  # if publisher name is in the certificate subject
                    signals.setdefault("publisher_bucket_hits", []).append(
                        bucket
                    )  # record which class this publisher maps to
        else:  # if not signed
            if in_system:  # and it is in system directory
                S -= float(
                    w["unsigned_system_penalty"]
                )  # big penalty (system files should be signed)
        ...
        if name:  # if we have a process name
            ...
            if len(name) >= int(w["hexish_name_len"]) and _looks_hexish(
                name
            ):  # if name is long and hex-like
                S -= 0.5  # penalty for hex-looking names (often packed malware)
                reasons.append("name looks hex/packed-like")  # add explanation
            if any(
                tok in name for tok in w["susp_name_tokens"]
            ):  # if name contains suspicious tokens
            ...
            for canonical, miss in w["misspell_tokens"]:  # loop through known misspellings
                if (
                    miss in name and canonical not in name
                ):  # if name contains misspelling but not correct spelling
                    S -= 0.3  # penalty for misspelled system process names (often malware)
                    reasons.append(
                        f"name looks like a misspelling of '{canonical}'"
                    )  # add explanation
        ...
        if file_ctime and exe:  # if we have file creation time and path
            ...
            if (
                age_days <= float(w["file_age_days_fresh"]) and not in_system
            ):  # if file is very new and not in system dir
        ...
        risky = set(
            int(p) for p in w["risky_ports"]
        )  # convert risky ports list to set for fast lookup
        ...
        if remote_count > 0:  # if there are outbound connections
            S += float(
                w["remote_bump"]
            )  # small boost (outbound is normal for apps, not necessarily evil)
            reasons.append("has remote connections")  # add explanation
            if remote_count >= int(w["remote_many_count"]):  # if many connections
                S += float(
                    w["remote_many_extra"]
                )  # extra boost (legitimate apps often have many connections)
        ...
        if parent_name or parent_exe:  # if we know the parent process
            parent_base = (
                _basename(parent_exe) if parent_exe else parent_name
            )  # get parent executable name
            if (
                parent_base and parent_base in w["susp_launchers"]
            ):  # if parent is a suspicious launcher
                S -= float(
                    w["susp_launcher_penalty"]
                )  # penalty for being launched by script interpreters
        ...
        if elevation and in_userish:  # if process is elevated and running from user/temp path
            S -= float(
                w["elev_from_user_penalty"]
            )  # big penalty (elevated malware often runs from temp)
        ...
        if is_service and in_userish:  # if process is a service running from user/temp path
            S -= float(
                w["service_userdir_penalty"]
            )  # big penalty (services should not be in user dirs)
        ...
        if sha256:  # if we have a file hash
            hs = self.db["hash_stats"].get(
                sha256, {"seen": 0, "last_seen": 0.0}
            )  # get previous stats for this hash
            ...
            eff = self._decayed_seen(
                max(seen_prev - 1, 0), last_seen, halflife
        ...
        if base:  # if we have an executable name
            ...
            # exclude current sighting from effective-seen (we are evaluating it right now)
            eff2 = self._decayed_seen(
                max(seen_prev2 - 1, 0), last_seen2, halflife
            )  # calculate decayed count
            signals["exe_eff_seen"] = eff2  # save for debugging
            if eff2 >= float(w["exe_seen_thresh"]):  # if we have seen this name many times before
                S += float(
                    w["prevalence_exe_bonus"]
                )  # smaller boost for common name (less reliable than hash)
        ...
        cls = self._classify(
            signals, signer_subject, is_service, parent_exe or parent_name
        )  # determine process class

        # convert internal S into a categorical verdict and a confidence
        verdict, confidence = self._to_verdict_and_confidence(
            S, w
        )  # map score to verdict and calculate confidence
    ...
    def _classify(
        ...
        parent_base = (
            _basename(_safe_lower(parent_path)) if parent_path else ""
        )  # get parent executable name
        parent_is_dev_shell = parent_base in {
            "powershell.exe",
            "pwsh.exe",
            "cmd.exe",
        }  # check if launched by dev shell

        # evaluate rule buckets in order; first match wins
        rules: dict[str, dict[str, Any]] = dict(
            w.get("class_rules", {})
        )  # get classification rules from weights
        for label, rule in rules.items():  # loop through rules in order
            ...
            if (
                rule.get("if_system_dir") and not in_system
            ):  # if rule requires system dir but process is not in system
                continue  # skip this rule, try next one
            if (
                rule.get("if_signed") and not publisher
            ):  # if rule requires signature but process is not signed
                continue  # skip this rule, try next one
            vend_list = [
                v for v in rule.get("if_publisher_any", [])
            ]  # get list of allowed publishers
            if vend_list and not any(
                v in publisher for v in vend_list
            ):  # if rule requires specific publisher but not found
                continue  # skip this rule, try next one
            if (
                rule.get("if_service") and not is_service
            ):  # if rule requires service but process is not a service
                continue  # skip this rule, try next one
            if (
                rule.get("if_parent_is_dev_shell") and not parent_is_dev_shell
            ):  # if rule requires dev shell parent but not found
                continue  # skip this rule, try next one
    ...
    def _to_verdict_and_confidence(self, S: float, w: dict[str, Any]) -> tuple[str, float]:
        ...
        if S < float(w["cut_malicious"]):  # if score is below malicious threshold
            ...
            confidence = min(
                1.0, 0.6 + (float(w["cut_malicious"]) - S) / 4.0
            ...
            confidence = (
                0.55 + (float(w["cut_suspicious"]) - S) / 6.0
            )  # calculate confidence (closer to threshold = lower confidence)
        elif S < float(w["cut_caution"]):  # else if score is below caution threshold
            verdict = "caution"  # verdict is caution
            confidence = (
                0.45 + (S - float(w["cut_suspicious"])) / 6.0
            )  # calculate confidence in caution range
        elif S < float(w["cut_trusted"]):  # else if score is below trusted threshold
            verdict = "trusted"  # verdict is trusted
            confidence = (
                0.55 + (S - float(w["cut_caution"])) / 6.0
            )  # calculate confidence in trusted range
        else:  # else score is above trusted threshold
            verdict = "trusted"  # verdict is still trusted (high trust)
            confidence = min(
                1.0, 0.8 + (S - float(w["cut_trusted"])) / 4.0
            )  # higher confidence for very positive scores
