...
def _ensure_list(x: StrOrList | None) -> list[str]:
    ...
    if isinstance(x, str):  # if it is a single string, wrap it in a list
        return [x]
    return [str(v) for v in x]  # if it is already a sequence, convert each item to string and return as list
...
class RulesEngine:
    ...
    def _load_rules(self) -> list[dict[str, Any]]:
        ...
        with open(self.path, encoding="utf-8") as f:  # open the JSON file as UTF-8 text
            ...
            return data if isinstance(data, list) else []  # make sure it is a list, otherwise return empty list

    def _match(self, when: dict[str, Any], event: dict[str, Any]) -> bool:
        # source match
        src = when.get("source")  # get the required source type from the rule
        if src and event.get("source") != src:  # if rule specifies a source and event does not match
        ...
        if "listening_port" in when:  # if the rule checks for listening ports
            need = bool(when["listening_port"])  # what the rule wants (True = must have, False = must not have)
            has = bool(event.get("listening_ports"))  # whether the event actually has listening ports
        ...
        # name_contains
        names = _ensure_list(when.get("name_contains"))  # get list of strings to search for in process name
        if names:  # if there are names to check
            evname = (event.get("name") or "").lower()  # get the event's process name (lowercase, empty string if missing)
            if not any(s.lower() in evname for s in names):  # check if any of the search strings are in the name
                return False  # none of them matched, so rule doesn't match

        # exe_contains
        exes = _ensure_list(when.get("exe_contains"))  # get list of strings to search for in executable path
        if exes:  # if there are paths to check
            evexe = (event.get("exe") or "").lower()  # get the event's executable path (lowercase, empty string if missing)
            if not any(s.lower() in evexe for s in exes):  # check if any of the search strings are in the path
                return False  # none of them matched, so rule doesn't match
        ...
        if port_in:  # if the rule specifies ports that must be in the event
            ev_ports = set(event.get("listening_ports") or [])  # get the event's listening ports as a set
            if not ev_ports.intersection(set(port_in)):  # check if any of the required ports are in the event
        ...
        if port_not_in:  # if the rule specifies ports that must not be in the event
            ev_ports = set(event.get("listening_ports") or [])  # get the event's listening ports as a set
            if ev_ports.intersection(set(port_not_in)):  # check if any of the forbidden ports are in the event
        ...
        if "any_remote" in when:  # if the rule checks for remote connections
            need_remote = bool(when["any_remote"])  # what the rule wants (True = must have, False = must not have)
            has_remote = bool(event.get("remote_addrs") or event.get("remote_endpoints"))  # check both possible field names for remote addresses
            if need_remote and not has_remote:  # rule wants remote connections but event doesn't have any
                return False  # doesn't match
            if not need_remote and has_remote:  # rule doesn't want remote connections but event has some
                return False  # doesn't match

        return True  # all checks passed, this rule matches the event

    def evaluate(self, event: dict[str, Any]) -> dict[str, str]:
        decision = {"level": "info", "reason": "no rule matched"}  # default decision if no rules match
        for rule in self.rules:  # loop through rules in order (first match wins)
            ...
            if self._match(when, event):  # check if this rule matches the event
                ...
                decision = {
                    ...
                    "reason": then.get("reason", rule.get("name", "rule triggered")),  # get the reason, fall back to rule name or generic message
                }
